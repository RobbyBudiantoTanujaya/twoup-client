...
namespace TwoUp.EditorTools
{
    ...
    public static class AsyncMatchesSceneBuilder
    {
        ...
        {
            var scene = UiKit.OpenOrCreateScene("AsyncList");
            ...
            var controller = IdempotentBuildUtil.GetOrAddComponent<AsyncMatchesController>(screen);
        ...
        private static AsyncMatchRowView CreateRowTemplate(Transform parent)
        {
            ...
            var button = IdempotentBuildUtil.GetOrAddComponent<Button>(row);
            button.targetGraphic = img;

            var view = IdempotentBuildUtil.GetOrAddComponent<AsyncMatchRowView>(row);
