...
1. **Protobuf contract v0 is the single source of truth.** It lives in `proto/twoup.proto`. C# is generated with `protoc` into `Assets/Scripts/Generated/` (regenerate with `tools/generate-protos.ps1`; runtime is a vendored `Google.Protobuf.dll` in `Assets/Plugins/Protobuf/`). **Do not modify the contract.** If something is missing (e.g. a leave-room message), add a `// TODO(contract):` comment where you needed it instead.
...
No art/animation/audio pass, no ads, no IAP, no deep links, no localization, no second game. Function over form.
