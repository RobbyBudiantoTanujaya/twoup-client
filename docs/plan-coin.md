...
1. Jalankan full rebuild semua scene via batchmode `-executeMethod` menu build existing → tidak ada error; commit perubahan scene bila builder menghasilkan diff. Lalu jalankan `BuildAll` sekali lagi (run kedua, tanpa perubahan lain) dan cek `git status` untuk kesembilan scene — lihat kriteria idempotensi literal di poin acceptance criteria di bawah; diff apa pun di run kedua = selidiki dan perbaiki builder, JANGAN dilonggarkan jadi perbandingan semantik.
...
- **Kriteria idempotensi UTAMA (literal, byte-level):** jalankan `Unity.exe -batchmode -quit -executeMethod TwoUp.EditorTools.SkeletonBuilder.BuildAll` dua kali berturut-turut (commit hasil run pertama, lalu jalankan run kedua tanpa perubahan lain) → `git status` harus bersih (byte-identik, tidak ada diff sama sekali) untuk kesembilan scene yang dibangun `BuildAll()`: `Boot`, `ConnectFour`, `Home`, `Invite`, `Queue`, `Voting`, `Result`, `AsyncList`, `Shop` (lihat `BuildAllScenePaths` di `SkeletonBuilderIdempotencyTests.cs`). Ini bar acceptance gate yang sebenarnya — SkeletonBuilder/SceneBuilder wajib find-or-create in-place (bukan Destroy+Instantiate) persis supaya Unity tidak me-renumber fileID/GUID di run kedua; kalau `git status` masih kotor setelah run kedua, itu bug builder yang harus diperbaiki, BUKAN alasan untuk melonggarkan kriteria ke perbandingan semantik.
- **Pemeriksaan TAMBAHAN (supplementary, bukan pengganti kriteria di atas):** `SkeletonBuilderIdempotencyTests.BuildAll_IsSemanticallyIdempotent` (`Assets/Tests/EditMode/SkeletonBuilderIdempotencyTests.cs`) boleh tetap hijau di EditMode batchmode sebagai pemeriksaan semantik (hierarki nama + set tipe component per scene sebelum/sesudah BuildAll() dijalankan ulang, mengabaikan fileID/GUID) — ini pemeriksaan pelengkap untuk membantu diagnosa kalau literal check gagal, TIDAK boleh dipakai sebagai satu-satunya acceptance criterion untuk idempotensi gate.

---

<!-- ccq:progress:start -->
## CCQ Progress — 13/13 selesai
...
- [x] `t_0717_1950_010706b9` Retrofit Home scene: coin balance + Daily Reward popup + Panel_GetCoins
- [x] `t_0717_1950_36cb426f` Retrofit Voting scene: per-card coin price + affordability disable + Panel_GetCoins
- [x] `t_0717_1950_7c7e4451` Retrofit Result scene: coin price on Btn_Rematch + affordability disable + Panel_GetCoins
- [x] `t_0717_1950_4ea3e0d6` Retrofit Shop scene: coin balance + price_coins + Watch Ad label
...
- [x] `t_0717_1950_bb08f418` GATE: coin economy client — full rebuild + audit
<!-- ccq:progress:end -->
